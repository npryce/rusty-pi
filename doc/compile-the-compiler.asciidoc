...
Compiling the Rust compiler for the Raspberry Pi. Compiling Hello
World and running it on the Pi.
...
is completely automated and very reliable.  Although it does take a
very _looooonnnnnngggggggg_ time.
...
Git is available in the package managers for most Linux distributions.
For example, on Ubuntu or Debian you can install it with the command:
...
=== Download the Raspberry Pi Tools
...
Then add the Raspberry Pi tools to your shell's command search path:
...
[source,sh]
-------------------------------------------------------------------
% ./configure --target-triples=arm-unknown-linux-gnueabihf --prefix=$HOME/pi-rust && make && make install
-------------------------------------------------------------------

This will take a long time to complete.  When it's finished, we can
test it by compiling a simple Rust program and running it on the Pi.
But until then, you've got more than enough time to go have a cup of
tea. Or two.
...
the Pi, we'll compile a traditional Hello World program.  Here's Hello
World in Rust:
...
Save this into a file called hello.rs.  To compile it for the Pi, make
sure that the directory
~/pi-tools/arm-bcm2708/gcc-linaro-arm-linux-gnueabihf-raspbian/bin is
...
% ~/pi-rust/bin/rustc/ \
        ...
        -C linker=arm-linux-gnueabihf-g++ \
        hello.rs
-------------------------------------------------------------------
...
We'll look at ways of doing that in the next chapter.
