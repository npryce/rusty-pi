...
general-purpose input/output, or GPIO for short. Each pin can be
controlled by software to read or send a digital signal -- that is,
read or set whether the voltage level at the pin is high or low.

The Raspbian operating system makes the GPIO pins available to
user-space programs as files in the /sys filesystem. Getting to grips
with GPIO programming can be pretty confusing at first because of the
multitude of ways in which GPIO pins are identified in the Raspberry
Pi documentation.
...
To avoid confusion this book only refers to GPIO pins by
their SoC numbers and does not use the names GP0 to GP7. The circuit
diagrams show how the SoC pin numbers relate to the physical header
pins.

So let's have a look at the circuit we need to build to blink an LED
with GPIO.
...
Now we understand the basics of GPIO in Rasbian, let's look at how to
write that in Rust.
